...
public:
    ...
    static bool loadConfig(const QString &filepath, AppConfig &appConfig, std::vector<Action> &actions,
                           std::vector<std::unique_ptr<Menu>> &menus);
    /// @brief Saves the current shared action library and menu references.
    static bool saveConfig(const QString &filepath, const AppConfig &appConfig, const std::vector<Action> &actions,
