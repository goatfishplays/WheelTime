/**
 * @file scheduler_tests.cpp
 * @brief Comprehensive scheduler tests.
 *
...
using namespace Application::TestFakes;
...
{
    return std::make_unique<Action>(std::move(items), "scheduler", "", "scheduler", channel);
}
...
int main()
{
    ...
    if (failed != 0)
    {
        std::cerr << failed << " scheduler test(s) failed\n";
        return 1;
    }

    std::cout << "Scheduler tests passed (" << std::size(cases) << " cases)\n";
    return 0;
}
